    ...
    public class ConditionsService {
        ...
        public async Task<string> ToReadable(ConditionRule rule) {
            if (rule is ConditionRule.EventConditionRule eventRule) {
                ...
                var e = await storage.GetEvent(equalityRule.PropertyID);
                return $"{e.EventValue.Value} = '{equalityRule.Value}'";
            }
            else if (rule is ConditionRule.PropertyInequalityRule inequalityRule) {
                var e = await storage.GetEvent(inequalityRule.PropertyID);
                return $"{e.EventValue.Value} <> '{inequalityRule.Value}'";
            }
            else {
